...
class GrammarEnforcer {
    ...
    getConjugatedForm(lemma, tense, plural) {
        if (plural && tense === "present")
            return lemma;
        return this.conjugationRules[tense]?.[lemma] || lemma;
    }
    enforceSubjectVerbAgreement(subject, verb) {
    ...
    enforceDeterminerNounAgreement(determiner, noun) {
...
class ProfessionalTextGenerator {
    constructor(ngramSize = 3, minSentenceLength = 5, maxSentenceLength = 20) {
    ...
    processCorpus(paragraphs) {
        paragraphs.forEach((paragraph) => {
            const doc = (0, compromise_1.default)(paragraph);
    ...
    determineGrammaticalRole(term) {
        if (term.tags.includes("Determiner"))
            return "determiner";
        ...
        if (term.tags.includes("Adjective"))
            return "adjective";
        if (term.tags.includes("Adverb"))
            return "adverb";
        if (term.tags.includes("Preposition"))
            return "preposition";
        return "other";
    }
    createGrammaticalTemplate(terms) {
        return terms
            .map((term) => term.grammaticalRole || term.chunk || "other")
            .join("->");
    }
    processWord(word) {
        const wordKey = `${word.normalized}-${word.tags.join("-")}-${word.grammaticalRole}`;
    ...
    processNgrams(words, chunks) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (i < words.length - 3) {
                const quadgramPrefix = `${words[i]} ${words[i + 1]} ${words[i + 2]}`;
                this.model.ngrams.quadgrams[quadgramPrefix] =
                    this.model.ngrams.quadgrams[quadgramPrefix] || {};
                this.model.ngrams.quadgrams[quadgramPrefix][words[i + 3]] =
                    (this.model.ngrams.quadgrams[quadgramPrefix][words[i + 3]] || 0) + 1;
            }
        }
        // Chunk sequences
        if (chunks.length > 0) {
    ...
    analyzeGrammarPatterns() {
        ...
        this.model.stats.grammarPatterns = Object.keys(this.model.ngrams.grammarPatterns).length;
    }
    calculateNgramProbabilities() {
        ...
        const totalChunks = Object.values(this.model.ngrams.chunkSequences).reduce((a, b) => a + b, 0);
    ...
    generateSentence() {
        ...
        while (attempts < 10) {
            ...
            const pattern = this.selectWeightedRandom(Object.entries(this.model.ngrams.grammarPatterns));
            const generatedWords = [];
            const roles = pattern.split("->");
            let context = [];
            for (const role of roles) {
                const candidates = Object.values(this.model.words).filter((word) => word.grammaticalRole === role ||
                ...
                const selected = this.selectWithContext(validCandidates, context);
    ...
    applyGrammarCorrections(words) {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            if (i > 0 && current.tags.includes("Verb")) {
                ...
                if (prev.tags.includes("Noun") &&
                    prev.tags.includes("Subject")) {
                    current = this.grammarEnforcer.enforceSubjectVerbAgreement(prev, current);
                }
            }
            ...
            if (i > 0 && current.tags.includes("Determiner")) {
                ...
                if (next?.tags.includes("Noun")) {
                    current =
                        this.grammarEnforcer.enforceDeterminerNounAgreement(current, next);
                }
            }
    ...
    filterByContext(candidates, context) {
        if (context.length === 0)
            return candidates;
        return candidates.filter((word) => {
            ...
            if (context.length >= 3 && this.ngramSize >= 4) {
                ...
                const quadProb = this.model.ngrams.quadgrams[quadPrefix]?.[word.normalized] || 0;
                if (quadProb > 0.05)
                    return true;
            }
            if (context.length >= 2 && this.ngramSize >= 3) {
                const triPrefix = context.slice(-2).join(" ");
                const triProb = this.model.ngrams.trigrams[triPrefix]?.[word.normalized] ||
                    0;
                if (triProb > 0.05)
                    return true;
            }
            if (context.length >= 1 && this.ngramSize >= 2) {
                ...
                const biProb = this.model.ngrams.bigrams[biPrefix]?.[word.normalized] || 0;
                if (biProb > 0.05)
                    return true;
            }
            return false;
    ...
    selectWithContext(candidates, context) {
        const scored = candidates.map((word) => {
            let score = word.frequency;
            if (context.length >= 3 && this.ngramSize >= 4) {
                const quadPrefix = context.slice(-3).join(" ");
                score +=
                    (this.model.ngrams.quadgrams[quadPrefix]?.[word.normalized] || 0) * 1000;
            }
            if (context.length >= 2 && this.ngramSize >= 3) {
                const triPrefix = context.slice(-2).join(" ");
                score +=
        ...
        return this.selectWeightedRandom(scored.map(({ id, score }) => [id, score]));
    }
    ...
    generateFallbackSentence() {
        ...
        const frequentPatterns = Object.entries(this.model.ngrams.grammarPatterns)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5);
    ...
    generateParagraph(sentenceCount = 5) {
        ...
        while (sentences.length < sentenceCount &&
            ...
            if (sentence.split(" ").length >= this.minSentenceLength &&
    ...
    saveModel(path) {
        fs_1.default.writeFileSync(path, JSON.stringify(this.model, null, 2));
    }
    loadModel(path) {
...
class LLMSimulator extends ProfessionalTextGenerator {
    constructor(ngramSize = 4, maxContextLength = 2048, temperature = 0.7) {
        super(ngramSize);
        this.contextWindow = [];
    ...
    getNgramCandidates(context) {
        ...
        if (Object.keys(candidates).length === 0) {
            Object.entries(this.model.ngrams.unigrams).forEach(([token, prob]) => {
    ...
    generateWithPrompt(prompt, maxTokens = 50) {
    ...
    applyTemperature(candidates) {
        if (this.temperature === 0) {
            ...
            const maxEntry = Object.entries(candidates).reduce((max, entry) => (entry[1] > max[1] ? entry : max), ["", -Infinity]);
